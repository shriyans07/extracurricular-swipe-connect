...
const AddActivity = () => {
    ...
    const handleGradeChange = (grade: string, checked: boolean) => {
        if (checked) {
            setFormData(prev => ({ ...prev, gradeLevels: [...prev.gradeLevels, grade] }));
        } else {
            setFormData(prev => ({ ...prev, gradeLevels: prev.gradeLevels.filter(g => g !== grade) }));
        }
    };
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            {grades.map(grade => (
                                <div
                                    key={grade}
                                    className="flex items-center space-x-2 bg-white border border-gray-300 rounded-md px-2 py-1"
                                >
                                    <Checkbox
                                        id={grade}
                                        checked={formData.gradeLevels.includes(grade)}
                                        onCheckedChange={(checked) => handleGradeChange(grade, checked as boolean)}
                                        className="border-gray-400 data-[state=checked]:bg-[var(--accent)] data-[state=checked]:border-[var(--accent)]"
                                        style={{ ['--accent' as any]: ACCENT }}
                                    />
                                    <label htmlFor={grade} className="font-poppins font-medium text-black">
                                        {grade}
                                    </label>
                                </div>
                            ))}
